// Myo-OSC, by Samy Kamkar (Edited by Maximilian Weber (MW), April 2016)
...
public:
    ...
    void onPose(myo::Myo* myo, uint64_t timestamp, myo::Pose pose)
    {
		...
		// Vibrate the Myo whenever we've detected that the user has made a fist. (Commented out, MW)
        //if (pose == myo::Pose::fist) {
        //    myo->vibrate(myo::Myo::vibrationShort);
        //}
    }
    // Transmit EMG Data (MW)
...
int main(int argc, char** argv)
{
    ...
    // Permanently unlock Myo (no double tap required to do gesture recognition) (MW)
    hob.setLockingPolicy(myo::Hub::lockingPolicyNone);

    // Enable EMG Streaming (MW)
